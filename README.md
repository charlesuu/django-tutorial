...
파이썬은 저번 학기 학교 수업에서 써보았지만 코딩 양이 많지 않아서, 학원에서 다룬 언어들에 비해 익숙하지 않다. Django하면서 써먹고, 이 쯤에서 다시 리마인드 할겸 예전에 배웠던 내용들을 다시 찾아보았다. 문법적으로는 C#이나 C보다 친절해서 막상 파이썬으로 코딩할 일이 많아지면 금방 익숙해 질 수 있을 것 같다. 이제 눈으로만 보고 까먹는건 그만하고 배울 때 제대로 배워놔야겠다..ㅎ

## 장고 설치 및 가상환경의 개념
강의에 등장하는 내용은 아니지만 장고 설치 과정에서 이것저것 찾아보다 가상환경의 개념에 대해 알게 되었다. 바로 [다른 프로젝트](https://github.com/charlesuu/first-Django-app)에 적용해보면서 익혔는데, 한 컴퓨터에서 여러 프로젝트를 만들 때 프로젝트 별로 가상환경을 구성하는 개념이었다. 프로젝트가 자신만의 패키지 및 인터프리터를 사용하면 프로젝트를 여러 개 개발 할 때 버전으로 차이로 인해 꼬일 여지가 생긴다. 이런 상황이 생기지 않게 프로젝트 별로 완전히 분리된 환경을 만들어주는 기능이었다. 복잡한 프로젝트를 여러 개 개발하게 되거나, 특정 버전의 패키지를 필요로하는 프로젝트를 하게 될 때 유용하겠다는 생각이 들었다.
